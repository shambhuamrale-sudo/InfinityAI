  ...
  return (
    <AnimatePresence>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[65] bg-slate-950/70 backdrop-blur-sm">
        <motion.div initial={{ x: 16, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: 16, opacity: 0 }} className="absolute right-4 top-4 w-full max-w-md rounded-[1.6rem] border border-white/10 bg-[#0B1120]/95 p-5 shadow-[0_30px_120px_rgba(2,6,23,0.64)]">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => setNotificationsOpen(false)} className="rounded-full border border-white/10 bg-white/5 p-2 text-slate-300"><X className="h-4 w-4" /></button>
          </div>
