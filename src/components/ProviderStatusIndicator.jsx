...
export default function ProviderStatusIndicator({ provider = 'openrouter', status = 'healthy', className = '' }) {
  ...
  return (
    <TooltipBubble label={`${provider} • ${status}`}>
        <motion.div whileHover={{ y: -2, scale: 1.01 }} className={`flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.05] px-3 py-2 text-sm text-slate-300 backdrop-blur-xl ${className}`}>
