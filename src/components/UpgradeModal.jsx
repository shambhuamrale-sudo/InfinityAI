  ...
  return (
    <AnimatePresence>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/80 px-4 backdrop-blur-xl">
        <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 20, opacity: 0 }} className="glass w-full max-w-lg rounded-[2rem] p-7">
          ...
          <h3 className="mt-4 text-2xl font-semibold tracking-tight text-white">Your free trial has reached its limit.</h3>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <button onClick={() => updateSubscription('starter')} className="rounded-full brand-gradient px-5 py-3 text-sm font-semibold text-white shadow-[0_12px_40px_-12px_rgba(129,140,248,0.6)] transition hover:brightness-110">Upgrade to Starter</button>
            <button onClick={() => setUpgradeModalOpen(false)} className="rounded-full border border-white/10 bg-white/[0.04] px-5 py-3 text-sm font-semibold text-slate-300 transition hover:bg-white/10">Keep browsing</button>
          </div>
