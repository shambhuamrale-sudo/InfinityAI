...
export default function AdminPage() {
  ...
  if (!auth.loading && (!auth.isAuthenticated || auth.user?.role !== 'admin')) {
    return (
      <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#050816] px-4 text-white">
        <BackgroundEffects />
        <div className="rounded-[1.75rem] border border-white/10 bg-[#0B1120]/85 p-8 text-center backdrop-blur-2xl">
          <ShieldCheck className="mx-auto h-12 w-12 text-amber-300" />
          <h1 className="mt-6 text-2xl font-semibold text-white">Admin access required</h1>
            ...
            return (
              <motion.div key={module.title} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: index * 0.06 }}>
                <GlassPanel hover className="p-6">
                  <div className="inline-flex rounded-2xl border border-indigo-400/20 bg-indigo-500/10 p-3 text-indigo-300"><Icon className="h-6 w-6" /></div>
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-[1.2fr_0.8fr]">
          <GlassPanel className="p-6">
            ...
            <div className="mt-4 grid gap-4 md:grid-cols-2">
              <label className="text-sm text-slate-400">Trial days<input value={trialDays} onChange={(event) => setTrialDays(event.target.value)} className="mt-2 w-full rounded-2xl border border-white/10 bg-[#050816] px-3 py-2 text-white" /></label>
              <label className="text-sm text-slate-400">Daily chat limit<input value={chatLimit} onChange={(event) => setChatLimit(event.target.value)} className="mt-2 w-full rounded-2xl border border-white/10 bg-[#050816] px-3 py-2 text-white" /></label>
              <label className="text-sm text-slate-400">Daily image limit<input value={imageLimit} onChange={(event) => setImageLimit(event.target.value)} className="mt-2 w-full rounded-2xl border border-white/10 bg-[#050816] px-3 py-2 text-white" /></label>
              <button onClick={saveConfig} className="mt-2 rounded-full bg-gradient-to-r from-indigo-500 to-fuchsia-500 px-4 py-2 text-sm font-semibold text-white">Save admin controls</button>
            </div>
          </GlassPanel>
          <GlassPanel className="p-6">
            <p className="text-sm uppercase tracking-[0.3em] text-indigo-300">Platform health</p>
            <div className="mt-4 rounded-[1.2rem] border border-white/10 bg-white/5 p-4 text-sm text-slate-400">{providerStatus}</div>
